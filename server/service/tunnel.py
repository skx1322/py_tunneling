...
def handle_public_request(public_ssl_sock):
    ...
    if client_tunnel_sock is None:
        print("[-] Error: Desktop client is not connected yet.")
...
def accept_desktop_client(TUNNEL_PORT):
    global client_tunnel_sock
    control_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    control_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    control_socket.bind(('127.0.0.1', TUNNEL_PORT))
    control_socket.listen(1)
    print(f"[*] Control Port open on {TUNNEL_PORT}: ")
    
    client_tunnel_sock, addr = control_socket.accept()
    print(f"[+] Desktop client connected: Connected with {addr}")

def open_gateway(TUNNEL_PORT, FORWARD_PORT):
    threading.Thread(target=accept_desktop_client(TUNNEL_PORT), daemon=True).start()
    ...
    public_socket.bind(('127.0.0.1', FORWARD_PORT))
    public_socket.listen(20)
    print(f"[*] Public Gateway listening securely on HTTPS://127.0.0.1:{FORWARD_PORT}")
